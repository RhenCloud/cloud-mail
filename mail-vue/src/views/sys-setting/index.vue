    ...
    <el-scrollbar class="scroll" v-if="!firstLoading">
        ...
        <div class="card-grid">
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div>
                  <span>SMTP {{ $t("configuration") }}</span>
                </div>
                <div>
                  <el-button class="opt-button" style="margin-top: 0" @click="openSmtpList" size="small" type="primary">
                    <Icon icon="ic:round-list" width="18" height="18" />
                  </el-button>
      ...
      <el-dialog class="resend-table" v-model="showSmtpList" :title="'SMTP ' + $t('configuration')">
        <el-table :data="smtpList">
          ...
          <el-table-column property="port" :label="'Port'" />
      ...
      <el-dialog v-model="showSetBackground" class="cut-dialog" @closed="closedSetBackground">
        <template #header>
      ...
      <el-dialog v-model="addS3Show" :title="t('s3Configuration')" width="340" @closed="resetAddS3Form">
        <form>
          ...
          <el-input
            class="dialog-input"
            type="text"
            :placeholder="setting.s3AccessKey || 'Access Key'"
            v-model="s3.s3AccessKey"
          />
          <el-input
            style="margin-bottom: 10px"
            type="text"
            :placeholder="setting.s3SecretKey || 'Secret Key'"
          ...
          <div class="s3-button">
            ...
            <el-button type="primary" :loading="settingLoading && !clearS3Loading" @click="saveS3">{{
              t("save")
            }}</el-button>
          </div>
        </form>
      ...
      <el-dialog v-model="emailPrefixShow" :title="t('emailPrefix')" @closed="resetEmailPrefix">
        ...
        <el-button type="primary" style="width: 100%" :loading="settingLoading" @click="saveEmailPrefix">{{
          $t("save")
        }}</el-button>
      </el-dialog>
    </el-scrollbar>
...
function getUpdate() {
  ...
  axios
    .get("https://api.github.com/repos/maillab/cloud-mail/releases/latest")
    .then(({ data }) => {
...
function openSmtpList() {
  // build list from setting
  const list = Object.keys(setting.value.smtpConfigs || {}).map((k) => ({
    domain: k,
    ...(setting.value.smtpConfigs[k] || {}),
  }));
  smtpList.value = list;
  showSmtpList.value = true;
}
...
function cleanSmtpForm() {
  smtpForm.domain = "";
  smtpForm.host = "";
  smtpForm.port = "";
  smtpForm.secure = false;
  smtpForm.startTls = true;
  smtpForm.credentials.username = "";
  smtpForm.credentials.password = "";
}
...
function saveSmtp() {
  ...
  const domain = smtpForm.domain.replace(/^@/, "").trim();
  const cfg = {
    host: smtpForm.host,
    port: Number(smtpForm.port) || 587,
    secure: !!smtpForm.secure,
    startTls: !!smtpForm.startTls,
    credentials: {
      username: smtpForm.credentials.username,
      password: smtpForm.credentials.password,
    },
  };

  const settingForm = { smtpConfigs: {} };
  settingForm.smtpConfigs[domain] = cfg;
  editSetting(settingForm);
}

function editSmtp(row) {
  smtpForm.domain = row.domain;
  smtpForm.host = row.host;
  smtpForm.port = row.port;
  smtpForm.secure = !!row.secure;
  smtpForm.startTls = row.startTls !== undefined ? !!row.startTls : true;
  smtpForm.credentials.username = row.credentials?.username || "";
  smtpForm.credentials.password = "";
  smtpFormShow.value = true;
}

function deleteSmtp(row) {
